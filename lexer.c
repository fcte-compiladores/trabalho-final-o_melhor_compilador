...
void print_token_list(struct lex_process *process) {
    ...
    for (int i=0; i < process->token_vec->count; i++){
        ...
        printf("TOKEN\tSY: %c\n", token->cval);
            break;
        case TOKEN_TYPE_DOLLAR_SIGN: // NOVO: Impressão para o '$'
            printf("TOKEN\tSY: %c\n", token->cval);
            break;
        default:
            printf("UNKNOWN TOKEN TYPE: %d\n", token->type); // Adicionado para depuração
            break;
        }
...
struct token *read_next_token() {
    ...
    case '\n':
        ...
        break;
    case '$': // NOVO: Tratamento para o cifrão
        nextc(); // Consome o '$'
        token = token_create(&(struct token){.type = TOKEN_TYPE_DOLLAR_SIGN, .cval = '$'});
        break;
    default:
