...
void scope_iteration_start(struct scope* scope) {
    ...
    if (scope->entities->flags & VECTOR_FLAG_PEEK_DECREMENT) vector_set_peek_pointer_end(scope->entities);
}
...
void* scope_iterate_back(struct scope* scope) {
    if (vector_count(scope->entities) == 0) return NULL;
...
void* scope_last_entity_from_scope_stop_at(struct scope* scope, struct scope* stop_scope) {
    if (scope == stop_scope) return NULL;
    
    void* last = scope_last_entity_at_scope(scope);
    if (last) return last;

    struct scope* parent = scope->parent;
    if (parent) return scope_last_entity_from_scope_stop_at(parent, stop_scope);
